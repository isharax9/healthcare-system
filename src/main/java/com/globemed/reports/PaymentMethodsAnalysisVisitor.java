...
public class PaymentMethodsAnalysisVisitor implements ReportVisitor {
    ...
    private void generateEfficiencyAnalysis() {
        reportContent.append("⚡ PAYMENT METHOD EFFICIENCY\n");
    ...
    private void generateTrendAnalysis() {
        ...
        if (mostEfficient != null) {
            reportContent.append(String.format("⚡ Most Efficient: %s (%.1f efficiency score)\n",
        ...
        if (paymentMethods.size() >= 5) {
            reportContent.append("🟢 Good payment method diversity\n");
        } else if (paymentMethods.size() >= 3) {
            reportContent.append("🟡 Moderate payment method options\n");
        } else {
            reportContent.append("🔴 Limited payment method options\n");
        }
        reportContent.append("\n");
    }
    ...
    private void generateRecommendations() {
        ...
        if (leastUsed != null && leastUsed.getTransactionCount() > 0) {
            ...
            if (utilizationRate < 5) {
                reportContent.append(String.format("  • Consider promoting %s (currently %.1f%% usage)\n",
                        leastUsed.getMethodName(), utilizationRate));
            }
        }

        if (mostEfficient != null) {
            reportContent.append(String.format("  • Promote %s for better efficiency\n",
        ...
        if (!hasOnlinePayment) {
            reportContent.append("  • Implement online payment portal\n");
        }
        if (!hasDigitalWallet) {
            reportContent.append("  • Consider digital wallet integration (Apple Pay, Google Pay)\n");
        }
        ...
        reportContent.append("\n⚡ EFFICIENCY IMPROVEMENTS:\n");
        reportContent.append("  • Automate payment processing where possible\n");
        reportContent.append("  • Reduce manual payment entry\n");
        reportContent.append("  • Implement real-time payment verification\n");
        reportContent.append("  • Provide payment method education to patients\n");

        // Security and compliance
        reportContent.append("\n🔒 SECURITY & COMPLIANCE:\n");
        reportContent.append("  • Ensure PCI compliance for card payments\n");
        reportContent.append("  • Implement fraud detection systems\n");
        reportContent.append("  • Regular security audits of payment systems\n");
        reportContent.append("  • Patient data protection measures\n");
